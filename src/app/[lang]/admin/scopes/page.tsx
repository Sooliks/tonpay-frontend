...
import CreateScope from "@/app/[lang]/admin/scopes/CreateScope";
import CreateSubScope from "@/app/[lang]/admin/scopes/CreateSubScope";
                        ...
                        {filteredData.map(scope=>
                            <AccordionItem value={scope.id} key={scope.id}>
                                ...
                                <AccordionContent>
                                    ...
                                    <p className={'text-1xl text-center mt-2'}>Sub categories: </p>
                                        {scope.subScopes.length > 0 ?
                                            <div className={'flex flex-wrap'}>
                                                {scope.subScopes.map(subScope=>
                                                    <Badge key={subScope.id} className={'mr-1 mb-1'}>{subScope.name}{subScope.isCurrency && ' $'}</Badge>
                                                )}
                                            </div>
                                            :
